...
function enqueue_google_fonts()
{
    wp_enqueue_style(
        'google-fonts',
        'https://fonts.googleapis.com/css2?family=Your+Font+Name:wght@400;700&display=swap',
        false
    );
}
...
add_filter('comment_form_fields', function ($fields) {
    ...
    if ($field) {
        ...
        $fields['comment_custom_fields'] .=
            '<label for="comment_custom_fields">' .
            esc_html($field['label']) .
            '</label><br>';
        foreach ($field['choices'] as $value => $label) {
            $fields['comment_custom_fields'] .=
                '<input type="radio" id="comment_custom_fields_' .
                esc_attr($value) .
                '" name="comment_custom_fields" value="' .
                esc_attr($value) .
                '"> ' .
                esc_html($label) .
                '<br>';
...
add_action('comment_post', function ($comment_id) {
    if (isset($_POST['comment_custom_fields'])) {
        update_comment_meta(
            ...
            sanitize_text_field($_POST['comment_custom_fields'])
        );
    }
});
...
add_filter(
    ...
    function ($comment_text, $comment) {
        $comment_option = get_comment_meta(
            $comment->comment_ID,
            'comment_custom_fields',
            true
        );
